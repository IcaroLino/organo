...
function Squad({ members, squad, changeColor, remove, favorite }) {
  return (
    (members.length > 0) && <section className='squad' style={{ backgroundColor: hexToRgba(squad.color, 0.6), backgroundImage: "url(/images/bg.png)" }}>
      ...
      <div className='members'>
        {members.map((member) => <MemberCard key={member.id} headerColor={squad.color} member={member} remove={remove} favorite={favorite} />)}
      </div>
    </section>
  );  
}
